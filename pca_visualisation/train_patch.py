...
class AdversarialPatch(nn.Module):
    def __init__(self, checkpoint_path, device, patch_size=16, px=0, py=0, threshold=0.3, temperature=10.0):
        super().__init__()
    ...
    def apply_patch(self, images):
        """Replace a region of each image with the adversarial patch."""
        x = images.clone()
        # Clamp patch to valid normalized RGB range
        patch_clamped = torch.stack([
            self.patch[c].clamp(NORM_MIN[c], NORM_MAX[c]) for c in range(3)
        ])
        x[:, :, self.py:self.py + self.patch_size, self.px:self.px + self.patch_size] = patch_clamped
        return x
    ...
    def forward(self, images):
        ...
        # Soft count of pixels above threshold (differentiable)
    ...
    def predict_heatmaps_patched(self, images):
...
def main():
    ...
    for epoch in range(1, args.e + 1):
        ...
        for images, _, metas in tqdm(loader, desc=f"epoch {epoch}/{args.e}", leave=False):
            ...
            # Forward + backward
            optimizer.zero_grad()
            soft_count = attacker(images)
            soft_count.backward()
            optimizer.step()

            # Monitoring: centers & recall (no grad)
        ...
        writer.writerow([epoch, f"{avg_loss:.3f}",
                          total_centers_clean, total_centers_patched,
                          f"{recall_clean:.4f}", f"{recall_patched:.4f}",
                          args.b, n_tiles])
        csv_file.flush()

        # Save on new best (lowest recall_patched)
