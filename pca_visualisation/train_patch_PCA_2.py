...
RESUME_SAVED_PATCH = None # "27_02-21_06_39_divulse-stern/patch32_distilled.pt"
    ...
    for step in tqdm(range(steps), desc="Steps"):
        ...
        for imgs, _, metas in tqdm(dataloader, desc=f"Step {step}", leave=False):
            ...
            with autocast("cuda"):
                ...
                for i in range(B):
                    # Directions
                    d32 = (f32[i] - fclean[i]).reshape(f32.shape[1], -1).T   # (N, C)
                    d64 = (f64[i] - fclean[i]).reshape(f64.shape[1], -1).T   # (N, C)

                    # PCA sur d64 (référence)
                    with torch.no_grad():
                        mean_i = d64.mean(dim=0, keepdim=True)               # (1, C)
                        d64c = d64 - mean_i                                  # (N, C)
                        d64c = torch.nn.functional.layer_norm(d64c, d64c.shape[1:])  # ou F.normalize(d64c, dim=1)

                        # SVD locale
                        U, S, Vh = torch.linalg.svd(d64c.float(), full_matrices=False)
                        W = Vh[:2, :]                                        # (2, C)

                    # Projections
                    p32 = (d32 - mean_i) @ W.T                               # (N, 2)
                    p64 = (d64 - mean_i) @ W.T

                    p32 = torch.nn.functional.layer_norm(p32, p32.shape[1:])
                    p64 = torch.nn.functional.layer_norm(p64, p64.shape[1:])

                    loss_pca += ((p32 - p64)**2).mean()
                ...
                for i in range(B):
                    ...
                    loss_dir += 1 - torch.nn.functional.cosine_similarity(d32, d64, dim=0)
